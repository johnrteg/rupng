...
export namespace Events
{
    ...
    /**
     * The platform's **canonical service registry** — the unique id of each deployable. It's the FIRST
     * segment of every `Action` (`service.noun.verb`) AND the identity the `Service`/`Job` base carries
     * (process/log name, the **`source`** of events on the bus, the resource-name prefix). Most members are
     * event-emitting services; a few (e.g. `WEBPROXY`) are non-emitting/local-only but still need an id.
     */
    export enum Service
    {
        APP          = "app",
    ...
    export interface Envelope
    {
        ...
        transactionId?: Type.ID;   // request-correlation id: stamped from the inbound API request's
                                  // `x-transactionid` and propagated onto every event emitted while handling it,
                                  // so the API → bus → downstream-services/sinks chain stays linked (monitor tracing)
