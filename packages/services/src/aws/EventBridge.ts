...
export class EventBridge
{
     ...
     */
    async put( busKey : ResourceKey, event : EventBridge.Event ) : Promise<void>
    {
        await this.putMany( busKey, [ event ] );
    }

    /** Emit several events in one call (≤ 10 — the PutEvents limit). */
    async putMany( busKey : ResourceKey, events : Array<EventBridge.Event> ) : Promise<void>
    {
