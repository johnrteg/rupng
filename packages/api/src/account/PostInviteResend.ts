//
import { RestfulEndpoint, Access, apiPath } from "@repo/endpoint";
import { NetworkUtils } from "@repo/common";
import { Account } from "./model/Account";

//
// Re-send a pending invite's (stub) email; bumps `lastSentAt` (the original `invitedAt` is preserved so
// the invite's age is unchanged). Admin-only. `{inviteId}` is the invite.
//
export class PostInviteResend extends RestfulEndpoint<PostInviteResend.Query, {}, PostInviteResend.Response>
{
    public readonly uri      : string = PostInviteResend.URI;
    public readonly method   : NetworkUtils.Method = NetworkUtils.Method.POST;
    public readonly access   : Access.Role = Access.AccountRole.ACCOUNT;
    public readonly timeout  : number | undefined = undefined;
    public readonly audience : RestfulEndpoint.Audience = RestfulEndpoint.Audience.APP;

    constructor( inviteId? : string ) { super( { inviteId: inviteId ?? "" }, {} ); }

    ////////////////////////////////////////////////////////////////////////////////////////////
    public getMappings(): Array<RestfulEndpoint.FieldMap> { return [ { field: "inviteId", location: RestfulEndpoint.AttrLocation.URI, required: true } ]; }
    public getQuerySchema(): RestfulEndpoint.Schema | null { return null; }
    public getBodySchema():  RestfulEndpoint.Schema | null { return null; }
}

export namespace PostInviteResend
{
    export const URI : string = apiPath( "acct", 1, "/invites/:inviteId/resend" );   // /api/acct/v1/invites/:inviteId/resend

    export interface Query { inviteId : string; }
    export interface Response { invite : Account.Invite; }

    export enum Error { UNAUTHORIZED = NetworkUtils.Status.UNAUTHORIZED, FORBIDDEN = NetworkUtils.Status.FORBIDDEN, NOT_FOUND = NetworkUtils.Status.NOT_FOUND, INTERNAL_SERVER_ERROR = NetworkUtils.Status.INTERNAL_SERVER_ERROR }
}

export default PostInviteResend;
