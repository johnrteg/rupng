...
export namespace Texting
{
    ...
    /**
     * The MMS compression tier a destination carrier needs (texting-2.6/media-10.10) — sourced from
     * Telnyx/Bandwidth/AWS MMS guidance: carrier byte caps aren't uniform, but they cluster into two bands,
     * so media only needs TWO pre-compressed variants (`MediaConfig.videoTargets["mms-relaxed"|"mms-safe"]`),
     * not one per carrier. `RELAXED` = Tier-1 carriers tolerate up to ~1MB; `SAFE` = the ~500-600KB universal
     * floor every carrier (and unresolved/unknown destinations) accepts.
     */
    export enum MmsTier { RELAXED = "mms-relaxed", SAFE = "mms-safe" }

    /** Which tier each tracked carrier needs (texting-2.6). `UNKNOWN` defaults to `SAFE` — never guess relaxed
     *  for a destination whose carrier couldn't be resolved. */
    export const MMS_TIER_BY_CARRIER : Record<Carrier, MmsTier> =
    {
        [ Carrier.ATT ]:         MmsTier.RELAXED,
        [ Carrier.VERIZON ]:     MmsTier.RELAXED,
        [ Carrier.TMOBILE ]:     MmsTier.RELAXED,
        [ Carrier.US_CELLULAR ]: MmsTier.SAFE,
        [ Carrier.UNKNOWN ]:     MmsTier.SAFE,
    };
    ...
    export enum MessageType { SMS = "sms", MMS = "mms", RCS = "rcs" }
    ...
    /**
     * One card in an RCS rich card / carousel (RCS Business Messaging). `title`/`mediaUrl` render the
     * card face; `suggestedReplies`/`suggestedActions` are the chip row underneath it.
     */
    export interface RcsCard
    {
        ...
        suggestedActions?:  Array<{ text: string; url?: Type.Url }>;
    }

    /**
     * RCS-specific rich content on an `OutboundRequest`/`SendRequest` — a single card's worth of fields
     * plus an optional `cards` array for a carousel. Only meaningful when the resolved adapter's
     * `SmsProvider.capabilities` includes `MessageType.RCS`; every RCS send still carries a plain `body`
     * fallback (`OutboundRequest.body`) since carrier/device RCS reachability isn't guaranteed per-recipient.
     */
    export interface RcsContent extends RcsCard { cards?: Array<RcsCard>; }

    /**
     * What ONE adapter can actually do with RCS — the fine-grained half of `SmsProvider.capabilities`
     * (texting-model rcs sketch). `carousel: false` means the adapter sends only the FIRST card of a
     * `RcsContent.cards` request (a documented fallback, e.g. Infobip today), not a real carousel — so
     * provider SELECTION (`SmsFactory.selectForRcs`) treats a multi-card request as disqualifying any
     * adapter whose limits don't fit it, rather than silently truncating it after the fact.
     */
    export interface RcsLimits
    {
        carousel:        boolean;    // true = sends a REAL multi-card carousel; false = first-card-only fallback
        maxCards?:        number;    // upper bound on `cards.length` when `carousel` is true
        maxSuggestions?:  number;    // upper bound on combined suggestedReplies + suggestedActions per card
        maxTitleLength?:  number;    // vendor's card-title character cap, when documented
    }
    ...
    export interface OutboundRequest
    {
        ...
        rcsContent?:    RcsContent;        // RCS rich card/carousel — SMS `body` above is the fallback text
    ...
    /**
     * A normalized inbound/DLR event after the adapter parsed its provider's dialect (texting-3.3). Every
     * adapter is responsible for mapping ITS OWN raw failure code onto the shared `ErrCode` vocabulary
     * (texting-6.5) — `errCode` is what the rest of the platform reasons about (suppression routing, number
     * health), while `providerCode` is retained VERBATIM so an operator can still see exactly what the vendor
     * said, including codes that don't map to any `ErrCode` (unmapped → `errCode` stays undefined,
     * `providerCode` alone carries the detail).
     */
    export interface NormalizedEvent
    {
        ...
        errCode?:           ErrCode;           // normalized failure category (texting-6.5) — undefined when not a failure or unmapped
        providerCode?:      string;            // raw vendor code, VERBATIM — audit + operator visibility, always preserved
        body?:              string;            // inbound
    ...
    export interface Message
    {
        ...
        rcsContent?:    RcsContent;        // RCS rich card/carousel actually sent (type === RCS)
    ...
    export interface SendRequest
    {
        ...
        rcsContent?:    RcsContent;        // requested RCS content — falls back to SMS `body` if the
                                            // resolved adapter/recipient can't take RCS (texting service decides)
