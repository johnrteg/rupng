...
export enum LambdaRuntime { NODE_20 = "nodejs20.x", NODE_22 = "nodejs22.x" }

export interface LambdaSpec
{
    key                  : ResourceKey;
    handler              : string;           // "file.export"
    runtime?             : LambdaRuntime;    // default NODE_22
...
{
    topic          : ResourceKey | { external : string };   // owned topic key, or an external topic name
    mode           : "publish" | "subscribe";
    consumerGroup? : string;                 // required for subscribe
}
...
export interface ServiceSpec
{
    ...
    desiredCount?   : PerEnv<number>;
    autoscaling?    : PerEnv<{ min : number; max : number; targetCpuPercent? : number; targetRequestsPerTarget? : number }>;
