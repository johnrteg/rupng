...
`@repo/cloud-manifest` lets you **describe what your service needs in plain TypeScript
...
Everything below is exported from `@repo/cloud-manifest`.
...
export const widgetManifest : ResourceManifest = {
    ...
    owns: {
        ...
        functions: [
            { key: "processor", handler: "processor.handler", runtime: JobRuntime.NODE_22,
...
```
   apps/<service>/src/infra.ts          <- your manifest (uses @repo/cloud-manifest types)
            │
            ▼
   /cloud  (the CDK app)                <- imports the manifest, builds real AWS
            │   uses @repo/cloud-manifest Naming to name things & publish identifiers
            ▼
   AWS                                  <- real resources + IAM + injected env vars
            │
            ▼
   apps/<service>/src/*.ts              <- your runtime code reads names back
                                           via CloudResolver (@repo/cloud-manifest)
```

The same `@repo/cloud-manifest` package is imported on all three rungs. That's the whole
...
```ts
import { Environment, CloudResolver } from "@repo/cloud-manifest";
