...
import type { Events } from "@repo/system";   // Kafka topic identity = Events.Object / Events.Stream
...
export interface CdnSpec
{
    ...
    // Host a built static site (SPA) from this bucket. `source` is the repo-relative path to the
...
{
    source : string;                         // repo-relative path to the built assets (vite outDir)
    spa?   : boolean;                        // default true — 403/404 → /index.html for client-side routing
}
...
// under `subscribes`.
//
// The `topic` IS the event identity, from @repo/system — never a literal:
...
export interface ServiceSpec
{
    ...
    // Container image. Default: CDK builds the repo-root Dockerfile (pushes to ECR, wires the task def).
    //   • image — a prebuilt registry/ECR image URI (CI pipelines) → skip the build, pull this instead.
    //   • build — Docker build-arg APP_PATH override (default `core/<service>`) for non-core layouts.
    image?          : string;
    build?          : string;
    tags?           : Tags;
}
