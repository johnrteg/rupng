...
cloud-spec `keys` entry) and consider **field-level envelope encryption** of the code body specifically.
...
This package (`packages/workflow`) is the right home for the **engine-agnostic core**: the
definition schema, node catalog + node interface, the expression evaluator, validation, and the
`Engine` interface. The **deployed service** (trigger router, worker, API, visualization data)
belongs in **`apps/core/workflow`** and imports this package — matching the repo's package/app split
(like `@repo/endpoint` vs the services that use it). *(Open decision: confirm this split vs a single
app.)*
...
5. **Package vs app split** — confirm `packages/workflow` (core) + `apps/core/workflow` (service),
   or build it as a single app first and extract the core later.
