...
import { Box, Drawer, Stack, ToggleButton, ToggleButtonGroup, Typography } from "@mui/material";
...
//
// EmailPreviewDrawer — the right-side email PREVIEW drawer: renders compiled HTML in a sandboxed iframe at a
// desktop or mobile viewport (a toggle resizes the drawer + frame). Presentation-only; the host owns the
// open/viewport state + the compiled HTML and passes them in. Shared by the template editor + the templates list.
//
export function EmailPreviewDrawer( props : EmailPreviewDrawer.Props ) : JSX.Element
{
    ...
    return <Drawer anchor="right" open={ props.mode !== null } onClose={ props.onClose }>
        <Box sx={{ width: props.mode === Email.PreviewViewport.MOBILE ? 420 : 760, maxWidth: "100vw", height: "100%", display: "flex", flexDirection: "column", transition: "width 0.2s" }}>
...
export namespace EmailPreviewDrawer
{
    export interface Props
    {
        mode    : Email.PreviewViewport | null;            // null = closed
