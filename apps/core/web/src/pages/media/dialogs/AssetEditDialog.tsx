...
//
// AssetEditDialog — edit a media asset's display name and tags (PatchAsset). The parent owns open/close and
// passes the asset in; this dialog owns its own form state and performs the mutation, returning success to the
// parent so it can snack + reload.
//
export function AssetEditDialog( props : AssetEditDialog.Props ) : JSX.Element
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////
    // save name + tags → true on success (dialog closes)
    async function onSave() : Promise<boolean>
    {
        if( name.trim() === "" ) return false;
        const reply : RestfulService.Reply<PatchAsset.Response> = await appmodel.server.fetch( new PatchAsset( props.asset.guid, { name: name.trim(), tags } ) );
