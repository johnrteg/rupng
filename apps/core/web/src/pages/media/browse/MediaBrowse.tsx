...
import BrowseResultCard from '@pages/media/browse/BrowseResultCard';
...
// dominant-color filter (named colors the stock providers accept; "any" = no filter). Adapters map to each
// provider's own color vocabulary.
const COLOR_ANY : string = "any";
const COLOR_CHOICES : Array<SelectInput.Choice> =
[
    { value: COLOR_ANY, label: "Any color" },
    { value: "red",     label: "Red" },
...
export function MediaBrowse( props : MediaBrowse.Props ) : JSX.Element
{
    ...
    const [color,setColor]           = React.useState< string >( COLOR_ANY );
    ...
    async function loadProviders() : Promise<void>
    {
        ...
        if( reply.ok && reply.data )
        {
            ...
            setSelected( reply.data.providers.map( ( info : Browse.ProviderInfo ) => info.provider ) );   // default: all enabled providers
        }
    ...
    async function search() : Promise<void>
    {
        ...
        const query : Browse.Query = {
            ...
            filters: { cost: cost as "free" | "paid" | "any", color: color === COLOR_ANY ? undefined : color },
        };
        // fetch returns a Reply (Result) — it never throws, so branch on .ok; no try/catch needed
        const reply : RestfulService.Reply<PostBrowseSearch.Response> = await appmodel.server.fetch( new PostBrowseSearch( query ) );
        if( reply.ok && reply.data )
        {
            setResults( reply.data.results ); setStatuses( reply.data.providers );
        }
        else
        {
            setResults( [] );
            setStatuses( [] );
            setSnack( { message: RestfulService.error( reply, "Search failed" ), severity: "error" } );
        }
        setSearching( false );
    }
    ...
    async function importResult( result : Browse.Result ) : Promise<boolean>
    {
        ...
        if( reply.ok )
        {
            setSnack( { message: "Added to your library.", severity: "success" } );
            return true;
        }
                            ...
                            <Stack spacing={ 2 }>
                                ...
                                <Stack direction="row" spacing={ 0.5 } sx={{ alignItems: "center", flexWrap: "wrap", gap: 1 }}>
                                    <SearchInput    id="browse-search"
                                                    ...
                                                    onChange={ setText }
                                                    onEnter={ () => void search() } />
                                    <SelectInput    id="browse-kind"
                                                    sx={{ width: 150 }} 
                                                    label={"Type"}
                                                    value={ kind }
                                                    choices={ KIND_CHOICES }
                                                    onChange={ setKind } />
                                    <SelectInput    id="browse-cost"
                                                     sx={{ width: 120 }}
                                                    label={"Cost"}
                                                    value={ cost }
                                                    choices={ COST_CHOICES }
                                                    onChange={ setCost } />
                                    <SelectInput    id="browse-color"
                                                    sx={{ width: 130 }}
                                                    label={"Color"}
                                                    value={ color }
                                                    choices={ COLOR_CHOICES }
                                                    onChange={ setColor } />
                                    <SelectMultInput id="browse-providers"
                                                    minWidth={ 220 }
                                                    dense={ true }
                                                    label={"Providers"}
                                                    value={ selected }
                                    ...
                                    <Button         variant="contained"
                                                    ...
                                                    disabled={ searching || text.trim() === "" || selected.length === 0 }
                                ...
                                { statuses.length > 0 &&
                                    <Stack direction="row" spacing={ 1 } sx={{ flexWrap: "wrap", gap: 1 }}>
                                        { statuses.map( ( bstatus : Browse.ProviderStatus ) =>
                                            <Chip key={ bstatus.provider } size="small" variant="outlined" color={ bstatus.ok ? "success" : "error" }
                                                  label={ bstatus.ok ? `${ bstatus.provider }: ${ bstatus.count }` : `${ bstatus.provider }: failed` } /> ) }
                                    </Stack> }
