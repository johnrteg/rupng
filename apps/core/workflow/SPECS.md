...
| **Visual flow editor (canvas)** | **buy** *(web)* | **React Flow (`@xyflow/react`)** — free/MIT **core** package, **no Pro subscription** ($283/mo tier) needed to build drag/zoom/pan/custom-node/edge-routing | The node-graph builder is a **web** concern; React Flow is the mature option (Rete.js / GoJS alternates). Workflow owns the schema + a layout hint, not the canvas. If a Pro-only feature becomes a hard requirement later, MIT terms permit forking/cloning just that piece rather than paying for the subscription — decide that if/when it actually comes up, not up front. |
