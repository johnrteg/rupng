...
const api =
{
    ...
    repoOutdated  : () : Promise<{ deps : NpmOutdated[]; error? : string }> => ipcRenderer.invoke( IPC.repoOutdated ),
    repoUpdateDeps : ( names : string[] ) : Promise<number> => ipcRenderer.invoke( IPC.repoUpdateDeps, names ),
    repoVersionConflicts : () : Promise<{ conflicts : VersionConflict[]; error? : string }> => ipcRenderer.invoke( IPC.repoVersionConflicts ),
    repoSyncVersions : ( names : string[] ) : Promise<number> => ipcRenderer.invoke( IPC.repoSyncVersions, names ),
