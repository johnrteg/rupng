...
import { Registration, RegistrationConfig } from "@repo/api";
import { ConfigSection } from "../configEditor/ConfigSection";

/** One configured carrier-provider entry — which CNP (carrier network provider), whether it's enabled, and
 *  the Secrets Manager entry (never an inline key) its credential lives under. Mirrors
 *  voiceConfig/ProviderRegistrySection.tsx's `ProviderRow`. */
function ProviderRow( props : ProviderRow.Props )
{
    /** Patch one field of this provider entry, preserving the rest. */
    function set( patch : Partial<RegistrationConfig.ProviderEntry> ) : void
    {
        ...
        <Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 1, p: 1, display: "flex", flexDirection: "column", gap: 1 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                ...
                <Select
                    ...
                    onChange={( event : SelectChangeEvent ) : void => set( { provider: event.target.value as Registration.CarrierProvider } )}
                    sx={{ minWidth: 130 }}
                >
                    {Object.values( Registration.CarrierProvider ).map( ( provider : Registration.CarrierProvider ) => <MenuItem key={provider} value={provider}>{provider}</MenuItem> )}
                </Select>
            ...
            <TextField
                ...
                helperText="Secrets Manager entry name (e.g. registration-bandwidth) — never the key itself" sx={{ minWidth: 260 }}
            />
        </Box>
    );
}
...
namespace ProviderRow
{
    ...
    {
        name     : string;
        value    : RegistrationConfig.ProviderEntry;
        onChange : ( value : RegistrationConfig.ProviderEntry ) => void;
...
/** The configured carrier-provider registry — the factory only offers ENABLED entries here as line-provisioning
 *  candidates (registration-6.1). */
export function CarrierProviderSection( props : CarrierProviderSection.Props )
{
    ...
    function onRename( oldName : string, newName : string ) : void
    {
        ...
        const next : Record<string, RegistrationConfig.ProviderEntry> = { ...props.value };
    ...
    function onEntryChange( name : string, value : RegistrationConfig.ProviderEntry ) : void
    {
    ...
    function onAdd() : void
    {
        ...
        props.onChange( { ...props.value, [ name ]: { provider: Registration.CarrierProvider.FAKE, enabled: false } } );
    }

    return (
        <ConfigSection title="Carrier provider registry" hint="Configured carrier network providers (CNPs). A credential only names a Secrets Manager entry — never an inline key.">
            {Object.entries( props.value ).map( ( [ name, entry ] : [ string, RegistrationConfig.ProviderEntry ] ) => (
                <ProviderRow
                    key={name} name={name} value={entry} readOnly={props.readOnly}
                    onRename={( newName : string ) : void => onRename( name, newName )}
                    onChange={( value : RegistrationConfig.ProviderEntry ) : void => onEntryChange( name, value )}
...
export namespace CarrierProviderSection
{
    export interface Props
    {
        value    : Record<string, RegistrationConfig.ProviderEntry>;
        onChange : ( value : Record<string, RegistrationConfig.ProviderEntry> ) => void;
        readOnly : boolean;
    }
}

export default CarrierProviderSection;
