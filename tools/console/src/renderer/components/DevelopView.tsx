            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, px: 1.5, py: 0.75, borderBottom: "1px solid", borderColor: "divider" }}>
                ...
                {proxyRunning
                    ...
                    : <Tooltip title="Run the webproxy locally — single HTTP origin that serves the SPA and proxies API/WS to services (no API Gateway/CloudFront)">
                        <Button size="small" color="success" variant="outlined" startIcon={<PlayArrowIcon />} onClick={() => void api.proxyStart( edgePort )}>Start edge</Button>
                      </Tooltip>}
                ...
                <Tooltip title="Re-read service ports/roles from Ports.ts + pick up new services in apps/core (no restart)">
                    <Button size="small" variant="outlined" startIcon={<FindReplaceIcon />} onClick={() => void rescan()}>Rescan ports</Button>
                </Tooltip>
            </Box>
