...
export namespace Workflow
{
    ...
    /**
     * The address of an active position — a `NodeId`, extended **inside loops** with the iteration
     * index at each enclosing loop (nested → a path). The frontier is keyed by this, and it's the
     * `nodePath` of the {@link ResumeToken} so a per-iteration completion resumes the right slot.
     * e.g. `"send-email"` · `"loop-1#3/send-email"` · `"loop-1#3/loop-2#0/send-email"`.
     */
    ...
    export enum DefinitionStatus
    {
        DRAFT     = "draft",
        PUBLISHED = "published",
        PAUSED    = "paused",       // published but triggers DISABLED — no NEW instances; in-flight continue
    ...
    {
        RUNNING    = "running",
        WAITING    = "waiting",      // ≥1 position parked on a signal/timeout; holds no compute
        COMPLETED  = "completed",
        FAILED     = "failed",       // a step exhausted retries with no error edge
        CANCELED   = "canceled",     // graceful, user-initiated
        TERMINATED = "terminated",   // force-stopped (jammed / runaway loop / wrong flow); in-flight step abandoned
    ...
    /**
     * One visited node in an instance's history — the **per-node outcome record** that makes a run
     * self-explanatory (looking at a running instance, this is "all the outcomes of nodes upstream").
     * Audit + replay + visualization.
     */
    export interface HistoryEntry
    {
        nodeId      : NodeId;
        nodePath?   : NodePath;             // set for loop bodies — node id + iteration index
        kind        : NodeKind;
        enteredAt   : Type.ISODateTime;
        status      : StepStatus;
        attempt     : number;
        chosenEdge? : EdgeLabel | string;   // which outgoing edge was taken (why it branched as it did)
        output?     : unknown;              // the typed value this node wrote to context
        error?      : RunError;
    }

    /** Present while a position is parked — how it parks and how it resumes. */
    export interface WaitState
    {
        /** Per-wait correlation key an inbound {@link Signal} matches to resume THIS position. */
        waitKey       : string;
    ...
    /**
     * One active position on the execution **frontier**. A flow branches and can run parallel paths
     * (`split`/`merge`, parallel `for-each`), so an instance can have **several** at once — this is
     * why a single `currentNodeId` cursor is insufficient. A position is `running` (engine will step
     * it) or parked (`wait` set, status `WAITING`) until its signal/timeout.
     */
    export interface ActivePosition
    {
        nodePath : NodePath;             // node id (+ enclosing loop iteration indices)
        wait?    : WaitState;            // set iff this position is parked
    }

    /**
     * A live loop frame (`for-each` / `while`) — the **authoritative, durable** loop state. The engine
     * keeps this in the instance (survives restart, dedups a late/duplicate completion); the per-item
     * delegated message carries only the **index** (in its `nodePath`) as the address into this frame,
     * never the state itself. Nested loops → a stack of frames.
     */
    export interface LoopFrame
    {
        nodeId        : NodeId;           // the loop node
        index         : number;          // current iteration (0-based)
        total?        : number;          // for-each: collection length (definite); absent for `while`
        itemVar?      : string;          // for-each: the per-item variable name
        maxIterations : number;          // the guard (REQUIRED for `while`)
        parallel      : boolean;         // for-each fan-out (many iterations in flight at once)?
        pending       : number;          // parallel join counter — iterations still in flight
        outputs       : Array<unknown>;  // per-iteration accumulated outputs, by index
    }
    ...
    export interface Instance
    {
        ...
        frontier      : Array<ActivePosition>;  // the active/waiting position(s) — NOT a single cursor (empty once terminal)
        loops         : Array<LoopFrame>;        // live loop frames (authoritative loop state; a stack for nesting)
        context       : Context;                 // the accumulating blackboard (all variables)
        history       : Array<HistoryEntry>;     // every visited node's outcome+output — the inspectable upstream record
    ...
    /**
     * The **resume token** threaded onto a delegated side-effect call (SQS to texting/email/…) and
     * **echoed back** on its completion, so the engine resumes the EXACT node/branch/iteration — not a
     * sibling from the same fan-out, and idempotently against a late/duplicate completion. Distinct
     * from the trace transaction id (that's cross-service tracing; this is resume-routing).
     */
    export interface ResumeToken
    {
        instanceId : Type.ID;
        nodePath   : NodePath;   // which node/branch/iteration to resume (node id + loop indices)
        waitKey    : string;     // the parked position's per-wait correlation key
    }

    /**
     * An external event that resumes a waiting position — an inbound reply/webhook (`RESUME`) or a
     * scheduled timeout wake (`TIMEOUT`). Routed two ways: by the **echoed `{instanceId, nodePath,
     * waitKey}`** (our own delegated completions), or — for external inbound that can't carry our id
     * (a contact's SMS reply) — by `(accountId, subject, waitKey)` via GSI → the waiting position.
     */
    export interface Signal
    {
        ...
        waitKey     : string;
        kind        : SignalKind;
        instanceId? : Type.ID;          // set when echoed back directly (else resolved via the waitKey/subject GSI)
        nodePath?   : NodePath;          // set when echoed back — the exact position to resume
