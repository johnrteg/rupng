...
import { LOG_STREAMS, type ClaudeMode, type LogLine, type LogStream, type ServiceRole } from "../../shared/types";
import { api } from "../api";
...
type TabValue = LogStream | "claude" | "jobs";
...
export function ConsoleView(
    { service, roles, runningStreams, claudeMode, onClaudeMode } :
    {
        service : string;
        roles : ServiceRole[];
...
{
    const [ tab, setTab ]         = useState<TabValue>( "build" );
                        ...
                        <Tab
                            ...
                            label={
                                <Box sx={{ display: "flex", alignItems: "center", gap: 0.7 }}>
                                    {STREAM_LABEL[ s ]}
                                    {runningStreams.has( s ) && <Box sx={{ width: 7, height: 7, borderRadius: "50%", bgcolor: "warning.main" }} />}
                                </Box>
                            }
                        />
                    ) )}
