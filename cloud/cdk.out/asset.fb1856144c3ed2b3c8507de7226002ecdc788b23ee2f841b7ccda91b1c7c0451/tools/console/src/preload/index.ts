...
import {
    ...
    type VpcLinkDiagnosis, type ProxyConfig,
...
const api =
{
    ...
    proxyStart       : ( config? : string ) : Promise<void> => ipcRenderer.invoke( IPC.proxyStart, config ),
    proxyStop        : () : Promise<void> => ipcRenderer.invoke( IPC.proxyStop ),
    proxyRestart     : ( config? : string ) : Promise<void> => ipcRenderer.invoke( IPC.proxyRestart, config ),
