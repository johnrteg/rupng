...
class ProcessManager extends EventEmitter
{
    ...
    startProxy( config? : string ) : void
    {
        // `npm run dev -- --config <name>` selects the env config (local/development/…); default local
        const args : string[] = [ "run", "dev", "--", "--config", config ?? "local" ];
        void this.run( WEBPROXY_ID, "runtime", "npm", args, serviceDir( WEBPROXY_ID ) );
    }

    /** Stop then re-launch the webproxy with the given config (waits for the port to free). */
    restartProxy( config? : string ) : void
    {
        this.kill( WEBPROXY_ID, "runtime" );
        setTimeout( () => this.startProxy( config ), 1200 );
    }
