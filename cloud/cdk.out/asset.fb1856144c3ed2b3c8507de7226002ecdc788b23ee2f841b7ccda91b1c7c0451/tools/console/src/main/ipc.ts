...
import { listProxyConfigs, readProxyConfig, writeProxyConfig } from "./proxyConfig";
...
export function registerIpc( getWindow : () => BrowserWindow | null ) : void
{
    ...
    ipcMain.handle( IPC.proxyStart, ( _e, config? : string ) => { processManager.startProxy( config ); } );
    ipcMain.handle( IPC.proxyStop, () => { processManager.kill( WEBPROXY_ID, "runtime" ); } );
    ipcMain.handle( IPC.proxyRestart, ( _e, config? : string ) => { processManager.restartProxy( config ); } );
