...
# The event envelope — moved to `@repo/system`

The platform's **one universal event body** is **`Events.Envelope`** — and it lives in
[`@repo/system`](../../system/README.md), **not here**. It moved because it's typed by `Events.Object` /
`Events.Verb` (the event vocabulary), which `@repo/common` can't depend on without a cycle. `@repo/system` is
...
fat-event / per-entity-topic rules: [`@repo/system`](../../system/README.md) and
