...
)
{
    ...
    return (
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
            ...
            {/* body */}
            {tab === "web"
                ? <Box sx={{ flexGrow: 1, minHeight: 0 }}><WebAppPanel service={service} /></Box>
                : tab === "api"
...
// ── Web tab: the deployed SPA's URL + open-in-browser (frontend services only) ───────────────────
function WebAppPanel( { service } : { service : string } )
{
    ...
    return (
        <Box sx={{ flexGrow: 1, minHeight: 0, overflow: "auto", p: 2 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>Deployed web app</Typography>
                <Box sx={{ flexGrow: 1 }} />
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1.25, alignItems: "flex-start" }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, width: "100%" }}>
                            ...
                            <Tooltip title="Copy URL">
                                <IconButton size="small" onClick={() => void navigator.clipboard.writeText( url )}><ContentCopyIcon sx={{ fontSize: 15 }} /></IconButton>
                            </Tooltip>
                        </Box>
                        <Button variant="contained" color="success" startIcon={<OpenInNewIcon />} onClick={() => void api.openExternal( url )}>
                            Open in browser
                        </Button>
                        <Typography variant="caption" sx={{ color: "text.disabled" }}>
                            Served via CloudFront (S3 origin). On LocalStack, CloudFront is partially emulated — if it doesn't load, the build is still in S3.
                        </Typography>
                    </Box>
                )
                : (
                    <Typography variant="caption" sx={{ color: "text.disabled" }}>
                        {err
                            ? `Couldn't read the web app URL: ${err}`
                            : "No deployed URL yet. Run Build + Deploy for this service, then Refresh — the URL comes from the stack's WebsiteUrl output."}
                    </Typography>
                )}
        </Box>
    );
