...
identity from **`@repo/system`** — never a hand-typed literal — so a publisher and a subscriber can't drift:
...
* **The body** — an **`Events.Envelope`** (`@repo/system`): `{ version, eventId, occurredAt, accountId, actor,
