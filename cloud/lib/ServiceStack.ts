...
import {
    ...
    AppConfigSpec, LambdaSpec, ApiSpec, ApiEndpointSpec, ApiAuthorizer, ThrottleSpec,
    DatabaseSpec, CacheSpec, ServiceSpec, KafkaTopicSpec, SesSpec, BatchJobSpec,
    ...
    AttrType, StreamViewType, BillingMode, LambdaRuntime, BucketAccess, CacheEngine,
...
export class ServiceStack extends cdk.Stack
{
    ...
    constructor( scope : Construct, id : string, props : ServiceStackProps )
    {
        ...
        // 3. Compute (env is complete by now), then user pools (triggers reference functions), then API.
        ( owns.functions ?? [] ).forEach( s => this.makeFunction( s ) );
    ...
    private makeFunction( spec : LambdaSpec ) : void
    {
    ...
    private makeEcsService( spec : ServiceSpec ) : void
    {
        ...
        const svc : ecsPatterns.ApplicationLoadBalancedFargateService = new ecsPatterns.ApplicationLoadBalancedFargateService( this, `Svc-${spec.key}`, {
            ...
            desiredCount         : this.per( spec.desiredCount, 1 ),
            ...
            taskImageOptions     : {
                // Placeholder image so synth works; point at the service's container build.
        ...
        const auto : { min : number; max : number; targetCpuPercent? : number; targetRequestsPerTarget? : number } | undefined = forEnv( spec.autoscaling, this.deployEnv );
        if( auto )
        {
    ...
    /** Map a spec `LambdaRuntime` to a CDK Lambda `Runtime` (defaults to Node 22). */
    private runtime( r : LambdaRuntime | undefined ) : lambda.Runtime
    {
        return ( r === LambdaRuntime.NODE_20 ) ? lambda.Runtime.NODEJS_20_X : lambda.Runtime.NODEJS_22_X;
    }
