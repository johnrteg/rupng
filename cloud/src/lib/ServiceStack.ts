        ...
        if( spec.access === BucketAccess.PUBLIC_CDN )
        {
            ...
            const dist : cloudfront.Distribution = new cloudfront.Distribution( this, `Cdn-${spec.key}`, {
                ...
                defaultBehavior : {
                    ...
                    // local has no trusted TLS — forcing HTTPS blocks asset loads behind the untrusted
                    // *.localhost.localstack.cloud cert; allow plain HTTP locally, redirect on real AWS.
                    viewerProtocolPolicy : isLocal( this.deployEnv )
            ...
            // Upload the built SPA into the bucket and invalidate the distribution on each deploy.
            if( site )
            {
                // the browsable URL: LocalStack serves CloudFront over plain HTTP through its edge port (4566)
                const siteUrl : string = isLocal( this.deployEnv )
                    ? `http://${dist.distributionDomainName}:4566`
