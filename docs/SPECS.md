...
**[`@repo/system`](../packages/system/README.md)** — the shared base both `@repo/endpoint` (the API/event contract)
...
* **Correlation.** The envelope carries a **`transactionId`** — stamped from the inbound API request's
  `x-transactionid` and propagated onto every event emitted while handling it, so the **API → bus →
  downstream-services/sinks** chain stays linked end-to-end (the [monitor](../apps/core/monitor/SPECS.md)
  tracing story).
* **Consuming.** A subscriber subscribes by `Events.Object`, gets a typed `Events.Of<O>`, switches on
  `event.verb`, and reads the typed `event.data` (the entity representation) — no casting. Worked example
  (typed subscriber + generic sink): [`@repo/system` README](../packages/system/README.md).
...
Authoritative treatment: **[`@repo/system`](../packages/system/README.md)** (the vocabulary — `Object`, `Verb`,
...
* **`@repo/system`** — `Object`/`Verb`/`Stream`/`Action`, `Envelope` (`object`+`verb`+`action`+typed `data`),
